...
object StatusBarUtils {
     ...
     * @param activity 目标activity
     * @param color    状态栏颜色值
     * @param alpha    状态栏透明度 @IntRange(from = 0, to = 255)
     */
    fun setColor(activity: Activity, @ColorInt color: Int, alpha: Int) {
    ...
    fun setGradientColor(activity: Activity, view: View) {
        ...
        activityWeakReference.get()?.let {
            ...
            setRootView(it, false)
            setTransparentForWindow(it)
            setPaddingTop(it, view)
        }
    }
    ...
    private fun isMIUI6(): Boolean {
        return try {
            ...
            val mtd: Method = clz.getMethod("get", String::class.java)
     ...
     * @param alpha     状态栏透明度 @IntRange(from = 0, to = 255)
     */
    private fun setTranslucentView(viewGroup: ViewGroup, @ColorInt color: Int, alpha: Int) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            val cipherColor: Int = cipherColor(color, alpha)
